...
def main():
    ...
    df_hotels.to_csv('hotels_sep.csv', index=False)
    df_reviewers.to_csv('reviewers_sep.csv', index=False)
    df_reviews.to_csv('reviews_sep.csv', index=False)

    # Rename columns and merge
    df_hotels.rename(columns={'average_score': 'hotel_average_score', 'median_score': 'hotel_median_score',
                              'number_of_reviews': 'hotel_number_of_reviews', 'max_review_one_day': 'hotel_max_review_one_day',
                              'first_review': 'hotel_first_review'})

    df_reviewers.rename(columns={'number_of_reviews': 'reviewer_number_of_reviews', 'average_score': 'reviewer_average_score',
    ...
    df_reviews.rename(columns={'score': 'review_score', 'score_deviation': 'review_score_deviation'})
